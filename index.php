...
for( $i= 1; $i<=$pAGE; $i++) {// Управляем количеством страниц которые обходим
    ...
    foreach ($user_arr as $key_user => $value_user) {
        ...
        foreach ($data->find('.user-params__value_tags a') as $key => $value) {
            $user_arr[$key_user]['tags'][md5(trim($value->plaintext))] = $value->plaintext;
            $all_tags[md5(trim($value->plaintext))] = trim($value->plaintext);
        }
    }
...
//Записываем данные со страницы в базу
    ...
    $del_tags = 'DELETE FROM lancers.tags AS T WHERE T.hash IN(';
    foreach ($user_arr as $key_user => $value_user) {
        $del = $del . "'" . $key_user . "',";
        ...
        if (isset($value_user['tags'])) {
            foreach ($value_user['tags'] as $key_tags => $val_tags) {
                ...
                $ins_user = $ins_user . "INSERT INTO lancers.link_user_tags( user_hash, tags_hash)  VALUES ( '" . $key_user . "'," . "'" . $key_tags . "');";
            }
        }
    }
